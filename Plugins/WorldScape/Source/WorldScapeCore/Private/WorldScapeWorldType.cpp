...
double WorldScapeWorld::GetLattitude(FVector Position,FRotator LattitudeRotation, bool FlatWorld, double PlanetScaleCode)
{
	if (!FlatWorld) 
	{
		FTransform NoiseTransform = FTransform(LattitudeRotation, FVector(0, 0, 0));
		FVector TransformedVector = NoiseTransform.TransformVectorNoScale(Position);
		return (1 - abs(TransformedVector.Z) / PlanetScaleCode);
	}
	else 
	{
		return 0.5;
...
void WorldScapeWorld::GetSnappedPosition(DVector& SnappedPosition, DVector& HardenNormal, FVector2D& SubPosition, bool NormalBeenChanged, double& StepSize, 
	bool& ForceUpdate, DVector PPos, DVector PlanetLocation, int Lod, double AltitudeMultiplier, double TriangleSize, double PlanetScaleCode, double GridAngle, 
	bool flatWorld, bool UpdateHarden)
{
	DVector Normal = HardenNormal.ToFVector();
	Normal.SnapNormal(GridAngle);
...
DVector ECEFToCube(DVector position, double PlanetScaleCode) 
{
