...
void AWorldScapeRoot::CheckCollisionDeletion(DVector HardenNormal, bool ResetCollisions)
{
	if (ResetCollisions)
	{
	...
	for (int32 p = 0; p < CollisionActorList.Num(); p++)
	{
		...
		WorldScapeWorld::GetSnappedPosition(SnappedPosition, HardenNormal, SubPosition, ResetCollisions, offset, bForceUpdate, ProjectedPosition,
			PlanetLocation, 0, 1, CollisionTriangleSize, PlanetScaleCode, GridAngle, bFlatWorld, true);
...
void AWorldScapeRoot::CollisionLodHandler(DVector HardenNormal, double AltitudeMultiplier, bool ResetCollisions, double GroundHeight)
{
	...
	if (IsValid(GetWorld()))
	{
		...
		bool bGenerateAllCollision = ((!bGenerateCollisionForAllPlayer && (GetNetMode() == ENetMode::NM_DedicatedServer || GetNetMode() == ENetMode::NM_ListenServer)) || bGenerateCollisionForAllPlayer);
		...
		else
		{
			...
			{
				CollisionActorList.Add(WorldToECEF(GetWorld()->GetFirstPlayerController()->GetFocalLocation()));//->GetPawn()->GetActorLocation()));
			}
		...
		for (int32 i = 0; i < 9; i++)
		{
			...
			if (MakeCollision)
			{
				...
				ColisionGeneration(CollisionLods[ID], this, OffSettedPos, PlanetLocation, FVector2D(0, 0), PlanetScaleCode, 0,
					CollisionTriangleSize, CollisionResolution + 1, PlayerDistanceToGround, HardenNormal, bFlatWorld);
...
void ColisionGeneration::CalculateNoise()
{
	...
	for (int32 i = 0; i < MainPatch.Vertices.Num(); i++)
	{
		...
		FNoiseData Data = RootRef->GetNoise(ECEFVertexPosition.ToFVector(), NoiseVolumeListVar, VolumeListVar, VolumeTransformArray);
